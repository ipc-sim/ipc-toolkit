...
class Inertia:
    ...
    def __init__(self):
        ...
        self.xhat[vertices[:, 0] > mid_x, 0] -= 0.2
        self.mass = igl.massmatrix(vertices, faces, igl.MASSMATRIX_TYPE_VORONOI)
...
B = ipctk.BarrierPotential(dhat, kappa, use_physical_barrier=True)
...
grad = (
    inertia.gradient(vertices) + B.gradient(C, collision_mesh, vertices)
    # + ...
)
...
while np.linalg.norm(grad) > 1e-3 and iter < 1000:
    ...
    while condition:
        ...
        curr_energy = (
            inertia(next_vertices) + B(C, collision_mesh, next_vertices)
            # + ...
        )
