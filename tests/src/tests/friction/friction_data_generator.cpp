...
FrictionComplexData friction_data_generator_with_pairwise()
{
    ...
    pairwise_friction[std::make_tuple(1, 2)] = {
        0.6, 0.4
    }; // Pairwise static/kinetic friction for material 1-2
    pairwise_friction[std::make_tuple(2, 3)] = {
        0.8, 0.6
    }; // Pairwise static/kinetic friction for material 2-3
    pairwise_friction[std::make_tuple(1, 3)] = {
        0.9, 0.7
    }; // Pairwise static/kinetic friction for material 1-3

    const double max_d = dhat - 2e-8;
    const double d = GENERATE_COPY(range(0.0, max_d, max_d / 10));

    // Utility lambda to configure collisions and edges
    auto configure_collisions =
    ...
    SECTION("point-triangle pairwise")
    {
        ...
        V1 = V0;
        V1.row(0) << 1, d + GENERATE(-1, 1, 1e-1), 0; // Point at t=1
