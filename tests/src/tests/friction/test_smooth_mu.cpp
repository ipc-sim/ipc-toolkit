...
TEST_CASE("anisotropic_mu_eff_sqrt_mu0_t0_sq_plus_mu1_t1_sq helper", "[friction][smooth-mu][anisotropic]")
{
    ...
    // Test x direction
    const auto [mu_s_eff_x, mu_k_eff_x] = anisotropic_mu_eff_sqrt_mu0_t0_sq_plus_mu1_t1_sq(
        tau_dir_x, mu_s_aniso, mu_k_aniso);
    CHECK(mu_s_eff_x == Catch::Approx(mu_s_aniso[0]).margin(MARGIN).epsilon(EPSILON));
    CHECK(mu_k_eff_x == Catch::Approx(mu_k_aniso[0]).margin(MARGIN).epsilon(EPSILON));

    // Test y direction
    const auto [mu_s_eff_y, mu_k_eff_y] = anisotropic_mu_eff_sqrt_mu0_t0_sq_plus_mu1_t1_sq(
        tau_dir_y, mu_s_aniso, mu_k_aniso);
    CHECK(mu_s_eff_y == Catch::Approx(mu_s_aniso[1]).margin(MARGIN).epsilon(EPSILON));
    CHECK(mu_k_eff_y == Catch::Approx(mu_k_aniso[1]).margin(MARGIN).epsilon(EPSILON));

    // Test diagonal direction
    const auto [mu_s_eff_diag, mu_k_eff_diag] = anisotropic_mu_eff_sqrt_mu0_t0_sq_plus_mu1_t1_sq(
        tau_dir_diag, mu_s_aniso, mu_k_aniso);
    const double expected_mu_s_diag = std::sqrt(
        mu_s_aniso[0] * mu_s_aniso[0] * 0.5 + mu_s_aniso[1] * mu_s_aniso[1] * 0.5);
    const double expected_mu_k_diag = std::sqrt(
        mu_k_aniso[0] * mu_k_aniso[0] * 0.5 + mu_k_aniso[1] * mu_k_aniso[1] * 0.5);
    CHECK(mu_s_eff_diag == Catch::Approx(expected_mu_s_diag).margin(MARGIN).epsilon(EPSILON));
    CHECK(mu_k_eff_diag == Catch::Approx(expected_mu_k_diag).margin(MARGIN).epsilon(EPSILON));
}

TEST_CASE("anisotropic_mu_eff_dtau helper", "[friction][smooth-mu][anisotropic-derivative]")
{
    static constexpr double EPSILON = 1e-4;
    static constexpr double MARGIN = 1e-6;
    ...
    CHECK(dmu_eff_dtau[0] == Catch::Approx(fd_dmu_eff_dtau[0]).margin(MARGIN).epsilon(EPSILON));
    CHECK(dmu_eff_dtau[1] == Catch::Approx(fd_dmu_eff_dtau[1]).margin(MARGIN).epsilon(EPSILON));
    ...
    const Eigen::Vector2d dmu_eff_dtau_zero = anisotropic_mu_eff_sqrt_mu0_t0_sq_plus_mu1_t1_sq_derivative(
        tau_zero, mu_aniso, mu_s_eff_zero);
