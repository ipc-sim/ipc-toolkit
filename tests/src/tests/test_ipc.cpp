...
TEST_CASE("Dummy test for IPC compilation", "[!benchmark][ipc]")
{
    ...
    Eigen::MatrixXd V;
    Eigen::MatrixXi E, F;
    bool success = tests::load_mesh(mesh_name, V, E, F);
    REQUIRE(success);
...
TEST_CASE("IPC shape derivative", "[ipc][shape_derivative]")
{
    nlohmann::json data;
    {
    ...
    // Mesh
    const Eigen::MatrixXi E = data["boundary_edges"];
    Eigen::MatrixXd X = data["boundary_nodes_pos"];
    Eigen::MatrixXd V = data["displaced"];
