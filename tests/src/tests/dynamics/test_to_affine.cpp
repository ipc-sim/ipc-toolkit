    ...
    "[to_affine][gradient][hessian]")
{
    ...
    // Random reduced DOFs; entries in [-1, 1] keep rotations inside the log map.
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        const Eigen::MatrixXd Ci = Mi.transpose() * Mi
            + Eigen::MatrixXd::Identity(andof, andof);
    ...
    const auto analytic_grad = [&](const Eigen::VectorXd& x_) -> Eigen::VectorXd {
        return to_affine->apply_gradient(x_, grad_y(to_affine->to_affine(x_)));
    };

    // --- Round trip ---------------------------------------------------------
    if (rigid_cov) {
        CHECK((to_affine->from_affine(to_affine->to_affine(x)) - x).norm() < 1e-9);
    } else {
