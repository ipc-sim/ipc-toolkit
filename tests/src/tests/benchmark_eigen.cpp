...
TEST_CASE("eigen vec dynamic vs static", "[!benchmark][eigen]")
{
    const Eigen::MatrixXd V = Eigen::MatrixXd::Random(100, 3);
    ...
    BENCHMARK("Static")
    {
        ...
        Eigen::Vector3d t0 = V.row(t0i).transpose();
    ...
    BENCHMARK("Dynamic")
    {
        ipc::Vector<double, -1, 3> p = V.row(vi).transpose();
        ipc::Vector<double, -1, 3> t0 = V.row(t0i).transpose();
