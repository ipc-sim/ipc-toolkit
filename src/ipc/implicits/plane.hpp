...
///
/// @param[in]  points  Point positions as rows of a matrix.
...
///     A function that takes a vertex ID (row numbers in points) and a plane
///     ID (row number in plane_origins) then returns true if the vertex can
///     collide with the plane. By default all points can collide with all
///     planes.
...
///
/// @param[in] points_t0 Point positions at start as rows of a matrix.
/// @param[in] points_t1 Point positions at end as rows of a matrix.
/// @param[in] plane_origins  Plane origins as rows of a matrix.
/// @param[in] plane_normals  Plane normals as rows of a matrix.
/// @param[in] can_collide
///     A function that takes a vertex ID (row numbers in points) and a plane
