...
namespace detail {
    ...
    /// where n = point_line_normal(p, e0, e1) is the unit normal associated
    /// with the directed edge (e0 -> e1) chosen consistently for the point p.
    /// Positive/negative sign indicates which side of the directed edge the
    /// point lies on.
    ///
    ...
    {
        return point_line_normal<T>(p, e0, e1).dot(p - e0);
    }

    /// Compute the gradient of the signed point-to-line distance with respect
    /// to all input coordinates packed as [p_x, p_y, e0_x, e0_y, e1_x, e1_y]^T.
    ///
    ...
    inline Eigen::Vector<T, 6> point_line_signed_distance_gradient(
        ...
        Eigen::ConstRef<Eigen::Vector2<T>> e1)
    {
        const Eigen::Vector2<T> n = point_line_normal<T>(p, e0, e1);
    ...
    /// distance with respect to all input coordinates packed as [p_x, p_y,
    /// e0_x, e0_y, e1_x, e1_y]^T.
    ///
    ...
    const Eigen::MatrixBase<DerivedE1>& e1)
{
    ...
    return detail::point_line_signed_distance<T>(p, e0, e1);
}
...
{
    using T = typename DerivedP::Scalar;
    return detail::point_line_signed_distance_gradient<T>(p, e0, e1);
}
...
{
    using T = typename DerivedP::Scalar;
    return detail::point_line_signed_distance_hessian<T>(p, e0, e1);
}
