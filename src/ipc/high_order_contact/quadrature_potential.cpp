    ...
    size_t& num_collision_pairs) const
{
    ...
    std::unique_ptr<HighOrderCollisionDict<PointType::VERTEX>> collisions =
    ...
    size_t& num_collision_pairs) const
{
    ...
    unordered_map<std::array<int, 3>, std::shared_ptr<HighOrderCollision>>
        pairs;
    num_collision_pairs = 0;
    ...
    if (edge_edge_distance(
        ...
        for (const auto& other_f : f_set) {
            ...
            case PointTriangleDistanceType::P_T: {
                ++num_collision_pairs;
                auto pair = std::make_shared<
                    HighOrderCollisionTemplate<Face3P1, Vertex3>>(
                    other_f, vid, mesh);
                insert_pair(pairs, std::shared_ptr<HighOrderCollision>(pair));
                break;
            }
    ...
    std::unique_ptr<HighOrderCollisionDict<PointType::EDGE>> collisions =
    ...
    size_t& num_collision_pairs) const
{
    ...
    std::unique_ptr<HighOrderCollisionDict<PointType::FACE>> collisions =
        std::make_unique<HighOrderCollisionDict<PointType::FACE>>();
    collisions->initialize(
