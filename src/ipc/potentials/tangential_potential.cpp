    ...
    const bool no_contact_force_multiplier) const
{
    ...
    // Compute Γ
    const MatrixMax<double, 3, TangentialPotential::ELEMENT_SIZE> Gamma =
        collision.relative_velocity_matrix(beta);

    // Compute T = ΓᵀP
    const MatrixMax<double, TangentialPotential::ELEMENT_SIZE, 2> T =
        Gamma.transpose() * P;
    ...
    const bool no_mu) const
{
    ...
    // Compute Γ
    const MatrixMax<double, 3, ELEMENT_SIZE> Gamma =
        collision.relative_velocity_matrix(beta);
    ...
    // Compute ∇τ = ∇T(x + u)ᵀv + T(x + u)ᵀ∇v
    MatrixMax<double, 2, ELEMENT_SIZE> jac_tau;
    if (need_jac_N_or_T) {
        jac_tau.resize(dim - 1, n);
        // Compute ∇T(x + u)ᵀv
        for (int i = 0; i < n; i++) {
    ...
    // + -μ N T τ [∇(f₁(‖τ‖)/‖τ‖)]
    J += T_times_tau * grad_mu_f1_over_norm_tau.transpose();

    // + -μ N f₁(‖τ‖)/‖τ‖ [∇T] τ
    if (need_jac_N_or_T) {
        const VectorMax2d scaled_tau = mu_f1_over_norm_tau * tau;
        for (int i = 0; i < n; i++) {
            // ∂J/∂xᵢ = ∂T/∂xᵢ * τ
            J.col(i) += jac_T.middleRows(i * n, n) * scaled_tau;
        }
    ...
    // + -μ N f₁(‖τ‖)/‖τ‖ T [∇τ]
    J += mu_f1_over_norm_tau * T * jac_tau;
    ...
    // done easily.
    J *= -collision.weight * collision.normal_force_magnitude;
