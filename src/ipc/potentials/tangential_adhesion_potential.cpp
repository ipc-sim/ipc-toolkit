...
{
    assert(mu_s == mu_k);
    return mu_k * tangential_adhesion_f0(x, eps_a());
}
...
{
    assert(mu_s == mu_k);
    return mu_k * tangential_adhesion_f1_over_x(x, eps_a());
}
...
{
    assert(mu_s == mu_k);
    return mu_k * tangential_adhesion_f2_x_minus_f1_over_x3(x, eps_a());
}
