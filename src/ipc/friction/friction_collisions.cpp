    ...
    const std::function<double(double, double)>& blend_mu)
{
    ...
    for (const auto& c_vv : C_vv) {
        FC_vv.emplace_back(
            c_vv, vertices, edges, faces, barrier, dhat, barrier_stiffness);
    ...
    for (const auto& c_ev : C_ev) {
        FC_ev.emplace_back(
            c_ev, vertices, edges, faces, barrier, dhat, barrier_stiffness);
    ...
    for (const auto& c_ee : C_ee) {
        ...
        FC_ee.emplace_back(
            c_ee, vertices, edges, faces, barrier, dhat, barrier_stiffness);
    ...
    for (const auto& c_fv : C_fv) {
        FC_fv.emplace_back(
            c_fv, vertices, edges, faces, barrier, dhat, barrier_stiffness);
