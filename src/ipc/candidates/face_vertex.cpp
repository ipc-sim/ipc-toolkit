    ...
    const PointTriangleDistanceType dtype) const
{
    return point_triangle_distance(
        positions.row(vertex_id), positions.row(faces(face_id, 0)),
        positions.row(faces(face_id, 1)), positions.row(faces(face_id, 2)),
        dtype);
}
    ...
    const double conservative_rescaling) const
{
    return point_triangle_ccd(
        // Point at t=0
        positions_t0.row(vertex_id),
        // Triangle at t=0
        positions_t0.row(faces(face_id, 0)),
        positions_t0.row(faces(face_id, 1)),
        positions_t0.row(faces(face_id, 2)),
        // Point at t=1
        positions_t1.row(vertex_id),
        // Triangle at t=1
        positions_t1.row(faces(face_id, 0)),
        positions_t1.row(faces(face_id, 1)),
        positions_t1.row(faces(face_id, 2)), //
...
{
    std::cout << positions_t0.row(faces(face_id, 0)).format(OBJ_VERTEX_FORMAT);
    std::cout << positions_t0.row(faces(face_id, 1)).format(OBJ_VERTEX_FORMAT);
    std::cout << positions_t0.row(faces(face_id, 2)).format(OBJ_VERTEX_FORMAT);
    std::cout << positions_t0.row(vertex_id).format(OBJ_VERTEX_FORMAT);
    std::cout << positions_t1.row(faces(face_id, 0)).format(OBJ_VERTEX_FORMAT);
    std::cout << positions_t1.row(faces(face_id, 1)).format(OBJ_VERTEX_FORMAT);
    std::cout << positions_t1.row(faces(face_id, 2)).format(OBJ_VERTEX_FORMAT);
    std::cout << positions_t1.row(vertex_id).format(OBJ_VERTEX_FORMAT);
    std::cout << std::flush;
}
