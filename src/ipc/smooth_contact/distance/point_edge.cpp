...
namespace ipc {
    ...
    {
        if constexpr (dim == 2)
            return Math<scalar>::sqr(Math<scalar>::cross2(e0 - p, e1 - p)) / (e1 - e0).squaredNorm();
        else
    ...
    template <typename scalar, int dim>
    Vector<scalar, dim> PointEdgeDistance<scalar, dim>::point_line_closest_point_direction(
        const Eigen::Ref<const Vector<scalar, dim>>& p,
        const Eigen::Ref<const Vector<scalar, dim>>& e0,
        const Eigen::Ref<const Vector<scalar, dim>>& e1)
    {
    ...
    template <typename scalar, int dim>
    Vector<scalar, dim> PointEdgeDistance<scalar, dim>::point_edge_closest_point_direction(
        const Eigen::Ref<const Vector<scalar, dim>>& p,
        const Eigen::Ref<const Vector<scalar, dim>>& e0,
        const Eigen::Ref<const Vector<scalar, dim>>& e1,
        const PointEdgeDistanceType &dtype)
    ...
    template <int dim>
    Eigen::Matrix<double, dim, dim*dim> PointEdgeDistanceDerivatives<dim>::point_edge_closest_point_direction_grad(
        const Eigen::Ref<const Vector<double, dim>>& p,
        const Eigen::Ref<const Vector<double, dim>>& e0,
        const Eigen::Ref<const Vector<double, dim>>& e1,
        const PointEdgeDistanceType &dtype)
    {
        ...
        Vector<T, dim> out = PointEdgeDistance<T, dim>::point_edge_closest_point_direction(pT, e0T, e1T);
        Eigen::Matrix<double, dim, dim*dim> grad;
        for (int i = 0; i < dim; i++)
            grad.row(i) = out(i).getGradient();
    ...
    template <int dim>
    std::array<Eigen::Matrix<double, dim*dim, dim*dim>, dim> PointEdgeDistanceDerivatives<dim>::point_edge_closest_point_direction_hessian(
        const Eigen::Ref<const Vector<double, dim>>& p,
        const Eigen::Ref<const Vector<double, dim>>& e0,
        const Eigen::Ref<const Vector<double, dim>>& e1,
        const PointEdgeDistanceType &dtype)
