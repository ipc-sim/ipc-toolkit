...
namespace ipc {
    ...
    public:
        ...
        static Eigen::Matrix<double, dim, dim*dim> point_edge_closest_point_direction_grad(
            const Eigen::Ref<const Vector<double, dim>>& p,
            const Eigen::Ref<const Vector<double, dim>>& e0,
            const Eigen::Ref<const Vector<double, dim>>& e1,
            const PointEdgeDistanceType &dtype = PointEdgeDistanceType::AUTO);

        static std::array<Eigen::Matrix<double, dim*dim, dim*dim>, dim> point_edge_closest_point_direction_hessian(
            const Eigen::Ref<const Vector<double, dim>>& p,
            const Eigen::Ref<const Vector<double, dim>>& e0,
            const Eigen::Ref<const Vector<double, dim>>& e1,
            const PointEdgeDistanceType &dtype = PointEdgeDistanceType::AUTO);
