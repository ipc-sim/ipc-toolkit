...
namespace ipc {
    ...
    template <typename scalar>
    scalar smooth_edge_edge_potential_pointwise(
        const Eigen::Ref<const Vector3<scalar>>& ea0,
        const Eigen::Ref<const Vector3<scalar>>& ea1,
        const Eigen::Ref<const Vector3<scalar>>& eb0,
        const Eigen::Ref<const Vector3<scalar>>& eb1,
        ...
        EdgeEdgeDistanceType dtype)
    {
        const scalar mollifier_threshold = 1e-3 * (ea1 - ea0).squaredNorm() * (eb1 - eb0).squaredNorm();
        const scalar cross_sqr_norm = (ea1 - ea0).cross(eb1 - eb0).squaredNorm();
        const scalar x_div_eps_x = cross_sqr_norm / mollifier_threshold;
        const scalar mollifier = x_div_eps_x < scalar(1.) ? (2 - x_div_eps_x) * x_div_eps_x : scalar(1.);
        const scalar dist_sqr = edge_edge_sqr_distance(ea0, ea1, eb0, eb1, dtype);

        return inv_barrier<scalar>(dist_sqr / params.eps, params.r) * mollifier;
    }
}