    ...
    : Primitive(id, alpha, beta)
{
    ...
    is_active_ = smooth_edge3_term_type(
        ...
        vertices.row(_vert_ids[2]), vertices.row(_vert_ids[3]), _alpha, _beta,
        otypes);
}
    ...
    const Eigen::Ref<const Vector12d>& x) const
{
    ...
    return smooth_edge3_term_template<T>(
               ...
               X.row(4), _alpha, _beta, otypes)
        .getGradient();
#else
    return std::get<1>(smooth_edge3_term_gradient(
        d, x.head<3>(), x.segment<3>(3), x.segment<3>(6), x.tail<3>(), _alpha,
        _beta, otypes));
#endif
}
    ...
    ORIENTATION_TYPES& otypes)
{
    ...
    otypes.tangent_type(0) =
        otypes.compute_type(-dn.dot(t0) / t0.norm(), alpha, beta);
    otypes.tangent_type(1) =
        otypes.compute_type(-dn.dot(t1) / t1.norm(), alpha, beta);
    ...
    const ORIENTATION_TYPES& otypes)
{
    ...
    return std::make_tuple(
        ...
        gradient * grad_val,
        gradient * hess_val * gradient.transpose() + grad_val * hessian);
}
    ...
    const ORIENTATION_TYPES& otypes)
{
    ...
    double tangent_term =
        smooth_edge3_tangent_term(dn, e0, e1, f0, f1, alpha, beta, otypes);
    double normal_term =
        smooth_edge3_normal_term(dn, e0, e1, f0, f1, alpha, beta, otypes);
    ...
    const ORIENTATION_TYPES& otypes)
{
    ...
    if (otypes.normal_type(0) != HEAVISIDE_TYPE::ONE
        ...
        normal_term = Math<scalar>::smooth_heaviside(
            (Math<scalar>::smooth_heaviside(dn.dot(n0) / n0.norm(), alpha, beta)
             + Math<scalar>::smooth_heaviside(
                 dn.dot(n1) / n1.norm(), alpha, beta)
             - 1),
            alpha, 0);
    }
