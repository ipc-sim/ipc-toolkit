...
namespace ipc {
    ...
    double Edge3::potential(const Eigen::Ref<const Vector3d> &d, const Eigen::Ref<const Vector12d> &x) const
    {
        return smooth_edge3_term_template<double>(d.normalized(), x.head<3>(), x.segment<3>(3), x.segment<3>(6), x.tail<3>(), _alpha, _beta, otypes);
    }
    Vector15d Edge3::grad(const Eigen::Ref<const Vector3d> &d, const Eigen::Ref<const Vector12d> &x) const
    {
        ...
        return smooth_edge3_term_template<T>(X.row(0) / X.row(0).norm(), X.row(1), X.row(2), X.row(3), X.row(4), _alpha, _beta, otypes).getGradient();
#else
        return std::get<1>(smooth_edge3_term_gradient(d, x.head<3>(), x.segment<3>(3), x.segment<3>(6), x.tail<3>(), _alpha, _beta, otypes));
#endif
    }
    Matrix15d Edge3::hessian(const Eigen::Ref<const Vector3d> &d, const Eigen::Ref<const Vector12d> &x) const
    {
#ifdef DERIVATIVES_WITH_AUTODIFF
        Vector15d tmp;
        tmp << d, x;
        ...
        auto X = slice_positions<T, 5, 3>(tmp);
        return smooth_edge3_term_template<T>(X.row(0) / X.row(0).norm(), X.row(1), X.row(2), X.row(3), X.row(4), _alpha, _beta, otypes).getHessian();
#else
        return std::get<2>(smooth_edge3_term_hessian(d, x.head<3>(), x.segment<3>(3), x.segment<3>(6), x.tail<3>(), _alpha, _beta, otypes));
#endif
    }
        ...
        ORIENTATION_TYPES &otypes)
    {
        ...
        const Vector3d t0 = PointEdgeDistance<double, 3>::point_line_closest_point_direction(f0, e0, e1);
        const Vector3d t1 = PointEdgeDistance<double, 3>::point_line_closest_point_direction(f1, e0, e1);
    ...
    std::tuple<double, Vector15d, Matrix15d>
        ...
        const ORIENTATION_TYPES &otypes)
    {
        ...
        for (int d : {0, 1})
        {
            ...
            if (otypes.normal_type(0) == HEAVISIDE_TYPE::VARIANT)
            {
                auto [y, dy, ddy] = negative_orientation_penalty_hess(e0 - f, e1 - f, sign * dn, alpha, beta);
                dy.tail<3>() *= sign;
                ...
                // f
                hessian.block<3, 3>(f_id, f_id) += ddy.block<3, 3>(0, 0) + ddy.block<3, 3>(3, 3) + ddy.block<3, 3>(0, 3) + ddy.block<3, 3>(3, 0);
                // dn & e0, e1
                hessian.block<3, 6>(0, 3) += ddy.block<3, 6>(3, 0);
                hessian.block<6, 3>(3, 0) += ddy.block<6, 3>(0, 3);
                // dn & f
                hessian.block<3, 3>(0, f_id) -= ddy.block<3, 3>(6, 0) + ddy.block<3, 3>(6, 3);
                hessian.block<3, 3>(f_id, 0) -= ddy.block<3, 3>(0, 6) + ddy.block<3, 3>(3, 6);
                // f & e0, e1
                hessian.block<6, 3>(3, f_id) -= ddy.block<6, 3>(0, 0) + ddy.block<6, 3>(0, 3);
                hessian.block<3, 6>(f_id, 3) -= ddy.block<3, 6>(0, 0) + ddy.block<3, 6>(3, 0);
            }
        }

        const double hess_val = Math<double>::smooth_heaviside_hess(value - 1, alpha, 0);
        const double grad_val = Math<double>::smooth_heaviside_grad(value - 1, alpha, 0);

        return std::make_tuple(Math<double>::smooth_heaviside(value - 1, alpha, 0), 
            gradient * grad_val, gradient * hess_val * gradient.transpose() + grad_val * hessian);
    }
        ...
        const ORIENTATION_TYPES &otypes)
    {
        ...
        for (int d : {0, 1})
        {
            ...
            {
                const auto [t, g] = PointEdgeDistanceDerivatives<3>::point_line_closest_point_direction_grad(f, e0, e1);
                const auto [tmp_val, tmp_grad] = opposite_direction_penalty_grad(t, -dn, alpha, beta);

                vals[d] = tmp_val;
                
                Vector<double, 12> gradient_tmp;
                gradient_tmp << -tmp_grad.tail<3>(), g.transpose() * tmp_grad.head<3>();
        ...
        //     // e0, e1
        //     grads[0].segment<6>(3) = tmp_grad.head<3>().transpose() * g.block<3, 6>(0, 3);
        //     // f0
        //     grads[0].segment<3>(9) = tmp_grad.head<3>().transpose() * g.block<3, 3>(0, 0);
        // }
        ...
        //     // e0, e1
        //     grads[1].segment<6>(3) = tmp_grad.head<3>().transpose() * g.block<3, 6>(0, 3);
        //     // f0
        //     grads[1].segment<3>(12) = tmp_grad.head<3>().transpose() * g.block<3, 3>(0, 0);
        // }

        return std::make_tuple(vals.prod(), vals[0] * grads[1] + vals[1] * grads[0]);
    }
        ...
        const ORIENTATION_TYPES &otypes)
    {
        ...
        for (int d : {0, 1})
        {
            ...
            if (otypes.tangent_type(d) != HEAVISIDE_TYPE::ONE)
            {
                const auto [t, g, h] = PointEdgeDistanceDerivatives<3>::point_line_closest_point_direction_hessian(f, e0, e1);
                auto [tmp_val, tmp_grad, tmp_hess] = opposite_direction_penalty_hess(t, -dn, alpha, beta);
                tmp_grad.tail<3>() *= -1;
                ...
                // f, e0, e1
                hessian_tmp.block<9, 9>(3, 3) = g.transpose() * tmp_hess.block<3, 3>(0, 0) * g +
                                            tmp_grad(0) * h[0] + tmp_grad(1) * h[1] + tmp_grad(2) * h[2];

                // mixed 2nd derivatives
                hessian_tmp.block<3, 9>(0, 3) = tmp_hess.block<3, 3>(3, 0) * g;
                hessian_tmp.block<9, 3>(3, 0) = g.transpose() * tmp_hess.block<3, 3>(0, 3);
        ...
        return std::make_tuple(vals.prod(), grads[0] * vals[1] + grads[1] * vals[0], 
                hesses[0] * vals[1] + hesses[1] * vals[0] + grads[0] * grads[1].transpose() + grads[1] * grads[0].transpose());
    }
        ...
        const ORIENTATION_TYPES &otypes)
    {
        ...
        Vector15d gradient = (weight * t_term) * n_grad + (weight * n_term) * t_grad;
        ...
        const ORIENTATION_TYPES &otypes)
    {
        ...
        auto [t_term, t_grad, t_hess] = smooth_edge3_tangent_term_hessian(dn, e0, e1, f0, f1, alpha, beta, otypes);
        // auto [n_term, n_grad, n_hess] = smooth_edge3_normal_term_hessian(dn, e0, e1, f0, f1, alpha, beta, otypes);
        double n_term = 1.;
        Vector15d n_grad = Vector15d::Zero();
        ...
        t_hess.topLeftCorner<3, 3>() += dn_hess[0] * t_grad(0) + dn_hess[1] * t_grad(1) + dn_hess[2] * t_grad(2);
        t_grad.head<3>() = dn_grad * t_grad.head<3>();

        n_hess.topRows<3>() = dn_grad * n_hess.topRows<3>();
        n_hess.leftCols<3>() = n_hess.leftCols<3>() * dn_grad;
        n_hess.topLeftCorner<3, 3>() += dn_hess[0] * n_grad(0) + dn_hess[1] * n_grad(1) + dn_hess[2] * n_grad(2);
        n_grad.head<3>() = dn_grad * n_grad.head<3>();
        ...
        Matrix15d hessian = (weight * t_term) * n_hess + (weight * n_term) * t_hess +
                            (weight * t_grad) * n_grad.transpose() + (weight * n_grad) * t_grad.transpose();
