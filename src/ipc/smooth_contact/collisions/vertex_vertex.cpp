...
SmoothVertexVertexCollision::SmoothVertexVertexCollision(
    ...
    : SmoothCollision<6>(
        std::min(primitive0_, primitive1_),
        std::max(primitive0_, primitive1_),
        dhat,
        mesh)
{
...
bool SmoothVertexVertexCollision::compute_types(
    const Vector12d& positions, const ParameterType& params)
{
    ...
    return smooth_point2_term_type(
               points.row(0), direc, points.row(2), points.row(3), params.alpha,
               params.beta)
        && smooth_point2_term_type(
               points.row(1), -direc, points.row(4), points.row(5),
               params.alpha, params.beta);
}

template <class scalar>
scalar SmoothVertexVertexCollision::evaluate_quadrature(
    const Vector12d& positions, ParameterType params) const
{
    ...
    return smooth_point_point_potential_2d<scalar>(
        ...
        points.row(4), points.row(5), params);
}

double SmoothVertexVertexCollision::compute_distance(
