...
{
    Vector2<scalar> direc =
        PointEdgeDistance<scalar, 2>::point_edge_closest_point_direction(
            p, e0, e1, PointEdgeDistanceType::AUTO);
    const scalar dist = direc.norm();
    direc = -direc / dist;

    const scalar mollifier_val =
        point_edge_mollifier<scalar>(p, e0, e1, dist * dist);
    return smooth_edge2_term<scalar>(direc, e1 - e0)
        * Math<scalar>::inv_barrier(dist / params.dhat, params.r)
        * mollifier_val
        * smooth_point2_term<scalar>(
               p, direc, x0, x1, params.alpha, params.beta);
}
} // namespace ipc