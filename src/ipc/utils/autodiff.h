...
struct DiffScalarBase {
    ...
    static inline size_t getVariableCount() { return m_variableCount; }
...
public:
    ...
    inline friend DScalar1 operator*(const DScalar1& lhs, const DScalar1& rhs)
    {
        // Product rule
        return DScalar1(
            lhs.value * rhs.value, rhs.grad * lhs.value + lhs.grad * rhs.value);
    }
...
/**
 * \brief Automatic differentiation scalar with first- and second-order
 * derivatives
 *
 * This class provides an instrumented "scalar" value, which may be dependent on
...
public:
    ...
    {
        return DScalar2(
            lhs.value + rhs.value, lhs.grad + rhs.grad, lhs.hess + rhs.hess);
    }
    ...
    friend DScalar2 acos(const DScalar2& s)
    {
        ...
        result.hess +=
            s.grad * s.grad.transpose() * s.value / (temp * temp * temp);
    ...
    friend DScalar2 atan2(const DScalar2& y, const DScalar2& x)
    {
        ...
        result.hess = (y.hess * x.value + y.grad * x.grad.transpose()
                       - x.hess * y.value - x.grad * y.grad.transpose())
            / denom;

        result.hess -=
            (y.grad * (x.value / denomSqr) - x.grad * (y.value / denomSqr))
            * (x.grad * (Scalar(2) * x.value) + y.grad * (Scalar(2) * y.value))
                  .transpose();
...
std::ostream&
operator<<(std::ostream& out, const DScalar2<Scalar, VecType, MatType>& s)
{
    out << "[" << s.getValue() << ", grad="
        << s.getGradient().format(
               Eigen::IOFormat(4, 1, ", ", "; ", "", "", "[", "]"))
