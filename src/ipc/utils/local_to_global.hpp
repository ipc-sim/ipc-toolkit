...
/// @param grad The global gradient to which the local gradient will be added. Must be of size at least max(ids) * dim, but can be larger.
template <typename IDContainer, int GlobalOrder = Eigen::RowMajor>
    ...
    Eigen::Ref<Eigen::VectorXd> grad)
{
    ...
    for (int i = 0; i < n_verts; i++) {
        if constexpr (GlobalOrder == Eigen::RowMajor) {
            ...
            for (int d = 0; d < dim; d++) {
                grad[n_verts * d + ids[i]] += local_grad[dim * i + d];
            }
        }
    }
}
    ...
    for (int i = 0; i < n_verts; i++) {
        for (int d = 0; d < dim; d++) {
            if constexpr (GlobalOrder == Eigen::RowMajor) {
                ...
                grad.coeffRef(n_verts * d + ids[i]) += local_grad(dim * i + d);
            }
        }
    }
}
            ...
            for (int k = 0; k < dim; k++) {
                for (int l = 0; l < dim; l++) {
                    if constexpr (GlobalOrder == Eigen::RowMajor) {
                        ...
                        triplets.emplace_back(
                            n_verts * k + ids[i], n_verts * l + ids[j],
                            local_hessian(dim * i + k, dim * j + l));
                    }
                }
            }
...
template <
    ...
    int GlobalOrder = Eigen::RowMajor>
            ...
            for (int k = 0; k < dim; k++) {
                for (int l = 0; l < dim; l++) {
                    if constexpr (GlobalOrder == Eigen::RowMajor) {
                        ...
                        triplets.emplace_back(
                            n_rows * k + row_ids[i], n_cols * l + col_ids[j],
                            local_jacobian(dim * i + k, dim * j + l));
                    }
                }
            }
                ...
                for (int l = 0; l < dim; l++) {
                    ...
                    if (val != 0) {
                        if constexpr (GlobalOrder == Eigen::RowMajor) {
                            ...
                            triplets.add_value(
                                0, n_verts * k + ids[i], n_verts * l + ids[j],
                                val);
                        }
                    }
                }
