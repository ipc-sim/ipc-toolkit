...
template <typename scalar> scalar cubic_spline(const scalar& x)
{
    const scalar y = 2. * x;
    if (abs(y) >= 2)
        return scalar(0.);
...
/// @param x
/// @return
template <typename scalar> scalar quadratic_spline(const scalar& x)
{
...
template <typename scalar> scalar mollifier(const scalar& x)
{
    if (x <= 0)
        return scalar(0.);
    if (x <= 1) {
...
// support is [0, 1]
template <typename scalar> scalar inv_barrier(const scalar& x, const double& r)
{
...
template <typename scalar> scalar L_ns(const scalar& x)
{
    if (x <= 0.)
        return scalar(0.);
    if (x >= 1.)
...
template <typename scalar>
Vector2<scalar> linear_solve(
    const Eigen::Ref<const Matrix2<scalar>>& A,
    const Eigen::Ref<const Vector2<scalar>>& b)
{
...
Eigen::Matrix<T, rows, dim, Eigen::ColMajor, max_rows, dim>
slice_positions(const Eigen::VectorXd& positions)
{
...
inline void my_finite_gradient(
